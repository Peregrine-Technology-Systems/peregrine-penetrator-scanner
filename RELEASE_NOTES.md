...
## Unreleased
- fix: pin `psych` to ruby 4.0.5's built-in 5.3.1 so CI doesn't recompile it from source (#945). psych is only a transitive dep (rdoc, `>= 4.0.0`); bundler resolved it to the newest rubygems version (5.4.0), which has no precompiled linux binary and builds from source — needing `yaml.h`/libyaml at install, which the CI agent lacks, so cold-agent `bundle install` failed (dev CI red: #1446/#1448/#1450). Ruby ships psych 5.3.1 precompiled as a default gem; pinning to it removes the source build entirely. Suite green on 5.3.1 (543 examples, 0). Durable class-fix (libyaml on the agent image) tracked in the infrastructure repo; the green-by-deferral that hid this from feature CI at #944. (#945)
