...
- feat: migrate scan VM provisioning from Docker-pull to Packer-baked GCE image (#903). `scanner-base` image in `peregrine-pentest-dev` (built by infra#3982/#3985) provides the full toolchain natively (nuclei 3.9.0, Ruby 4.0.5, Node.js 22 LTS, ZAP, sqlmap, nikto, ffuf, testssl, retire.js, trufflehog, amass). All environments now clone the appropriate git branch at boot and run `bundle exec bin/scan` directly — no Docker pull, no Artifact Registry auth, no 10-15 min image-pull window. `trigger-scan.sh` resolves the baked image from the `scanner-base--vm-base-image` SM pointer; `cloud/scheduler/main.py` resolves it via the `scanner-base` image family. VM startup script writes `control/${SCAN_UUID}/versions.json` to GCS; smoke-test verifies nuclei/ruby/node are at expected Packer-baked versions as a positive toolchain assertion. Scavenger updated from `docker ps` to `pgrep -fa "bundle exec"` for liveness check. (#903)
