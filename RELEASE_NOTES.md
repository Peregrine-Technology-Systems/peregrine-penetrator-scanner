...
## Unreleased
- fix: repoint `.woodpecker/{sync-back,version-bump,bake,promote}.yaml` from the decommissioned `gs://ci-runners-de-agent-hooks` bucket to `gs://peregrine-production-agent-hooks` (#1193). Flagged via a cross-repo infrastructure notice — the old bucket is gone, so the next version-bump/promote/sync-back/bake run would have failed fetching `get-gh-app-installation-token.sh`. Verified all four call sites before fixing, not just the one named in the notice. (#1193)
- feat: adopt peregrine-bus#70's shared `Peregrine::Bus::Gcp::SmKeyProvider` (gcp-v0.3.0), retire the interim `Bus::KeysetFetcher` (#1189). Checked the shipped source before wiring it in — infra's summary said it took "the subject set," but it's actually scoped to exactly one subject per instance (`current_key_id` raises for any other). Flagged the discrepancy, then built `Bus::CompositeKeyProvider` — a small wrapper fanning the KeyProvider duck-type across N single-subject `SmKeyProvider`s — so scanner's one `Adapter` still serves all 3 data-plane subjects from one `key_provider`. Bumped `peregrine_bus_gcp` `~> 0.1` → `~> 0.3` (verified the tag is real before adopting), which pulled `google-cloud-secret_manager` down to `~> 1.5` (the gem's own transitive constraint) — verified the API `NatsCredsFetcher` depends on is unchanged across that range. `NatsCredsFetcher` itself is unaffected — infra confirmed the Synadia `.creds` fetch is a separate concern, not covered by #70. (#1189)
- fix: `KeysetFetcher`/`NatsCredsFetcher` — correct the project default + key-material decode against infra's confirmed contract (#1187). #1183/#1184 were a placeholder design pending infra's answers; infra confirmed and I verified against the authoritative reference (`peregrine-infrastructure`'s `seal_and_publish_analysis_requested.py` fn `resolve_seal_key`, not just their summary) — both fetchers now default to project `peregrine-production` (was `peregrine-pts-penetrator`), `NatsCredsFetcher` reads the real shared secret `peregrine-production--synadia-creds` (was a guessed per-class name), and `KeysetFetcher` now mirrors `resolve_seal_key`'s exact fallback: key material is RAW 32 bytes primarily, a base64 form accepted only if it decodes to exactly 32 bytes, anything else raises loud. The prior code assumed base64-always, which would have corrupted or crashed against the real raw-bytes secrets. Noted `peregrine-bus#70` (the shared SM-backed KeyProvider infra is landing in the gem itself) — this class is explicitly the interim implementation, not to be maintained in parallel once #70 ships. Infra also confirmed the SA grants (their own tracked gate) are applied and verified live. (#1187)
- feat: design pass for the self-fetching TP model (#1183, infra's scanner#1182 / arch#672). Under tp-monitor's MIG-resize model, nothing is injected at boot — the launcher-injected `/dev/shm/keyset.json` + `NATS_CREDS` file model is a confirmed dev-environment artifact. Added `InstanceMetadata.on_gce?` as the discriminator between a real deployed TP (self-fetch mandatory, fail-loud) and local dev/CI (existing #1106 fail-soft behavior, byte-for-byte unchanged). `Bus::KeysetFetcher`/`Bus::NatsCredsFetcher` self-fetch the bus keyset + Synadia creds from Secret Manager via the instance SA's ADC; `Bus::AdapterEnv` wires them in on GCE with construction errors re-raising instead of degrading to nil — an unprovisioned TP must crash at boot, never run silently off-bus. Secret naming is a clearly-marked PLACEHOLDER pending infra confirmation (asked directly); telemetry-plane subject keying is an open question, not guessed. Adds `google-cloud-secret_manager`. (#1183)

## v1.12.0 — 2026-07-20
- fix: `WorkerLoop`'s exiting event used the wrong subject shape — heartbeat segment + payload state, not `.exiting.` (#1173). Caught by analyzer before this reached staging/main: a separate `telemetry.tp.scanner.exiting.<tp-id>` subject is silently dropped by tp-monitor#764's consumer, which filters `telemetry.tp.<class>.heartbeat.<instance>` and reads lifecycle state from the PAYLOAD (arch#649: "state is a payload branch-point, not a subject segment"). `publish_exiting` now rides the same heartbeat subject `ControlPlaneLoop` already uses, with `state: 'exiting'` + `reason: 'idle_timeout'|'sigterm'` in the payload. Also aligned the idle-timeout env var precedence with analyzer#243's convention: per-class `SCANNER_DRAIN_IDLE_SECONDS` → shared fleet-wide `TP_DRAIN_IDLE_SECONDS` → default 480. (#1173)
- feat: convert `bin/scan` into a persistent idle-poll worker under `Bus::WorkerLoop` (#1171). Matches the shared analyzer#243/tp-monitor#733 convention — tp-monitor confirmed `txn-scanner-app` was still a run-once fixture (single pull-or-fallback at boot, no retry, no self-exit), same as analyzer's pre-#215 state. `Bus::WorkerLoop` now: boots → polls its own `scan.requested` subject → runs one job per pull via the extracted `ScanJobRunner` (the full target/scan/orchestrate/export/publish pipeline, pulled out of `bin/scan` so it's callable once per loop iteration instead of once per process) → resets its idle timer on each job → after `SCANNER_DRAIN_IDLE_SECONDS` (default 480 = tp-monitor's prod `drain_idle_seconds=300` + `cooldown_scale_down_seconds=180`, same number analyzer landed on) of no new work, publishes a worker-level exiting event and exits 0 (subject/shape corrected in #1173, see below) — ahead of tp-monitor's own MIG resize-to-0 backstop. SIGTERM traps into a `drain!` flag checked between pulls (never mid-job) for a graceful finish-in-flight shutdown. Off-bus (local dev/CI/smoke-test harness, `Bus::AdapterEnv.enabled?` false) is completely unchanged — single-shot CLI/ENV-driven run via the same extracted `ScanJobRunner`, preserving the existing smoke-test exit-code contract. (#1171)
- fix: drop `smoke:true` `scan.requested` messages instead of scanning them (#1166). Orchestrator confirmed `smoke` is a message-provenance marker ("synthetic transport-probe, discard without acting"), orthogonal to `profile` — never map it to scanner's own smoke-test profile, never scan a real target for a message meant to be discarded. `Bus::ScanRequestConsumer#next_request` now acks (existing ack-regardless semantics) and drops `smoke: true` payloads, returning nil so `bin/scan` falls through exactly as if no bus message existed. Belt-and-suspenders — `smoke:true` shouldn't reach this subject under current cross-repo code, but costs nothing to guard. (#1166)
- fix: correct `scan.requested` payload field mapping against the orchestrator's real contract (#1164). Orchestrator's `LifecycleEmitter` publishes `{job_id, status, transaction_id, environment, smoke, target_url, profile}` — `bin/scan` was reading `scan_uuid` (real key: `job_id`), `target_name` (doesn't exist in the real contract), and `urls` (real key: `target_url`, singular). Every one of those digs returned nil on a real payload, silently falling through to CLI/ENV defaults — the first live bus-triggered scan would have scanned `http://localhost:8080` instead of the real target. Added `Bus::ScanRequestPayload` to normalize the real shape: `target_url` wraps into a single-element `target_urls`, `target_name` is derived from the URL host (best-effort, no such field exists upstream). Subject and ack behavior were already correct — verified via source, not just claim. (#1164)
- fix: restore `Bus::ScanRequestConsumer` — the on-VM consumer PR #1117 removed against a now-superseded model (#1152). `tp-monitor#733` reversed the interim launcher-consumes-and-hands-off model that #1117 was built against: the canonical worker contract is now "the worker pulls, processes, and acks its own subject directly" — tp-monitor only counts backlog and resizes the MIG, it never brokers a message on a worker's behalf. Re-added `Bus::ScanRequestConsumer` (unchanged from its pre-#1117 shape, pulling `data.task.penetrator.scan.requested` off the adapter) and wired it back into `bin/scan` ahead of the `Bus::JobPointerResolver` claim-check path (`#1124`, itself built for the now-superseded model — left as a harmless no-op fallback pending a follow-up cleanup issue rather than removing it here) and the ENV/CLI fallbacks. Corrected `Bus::Publisher`'s doc comment, which still described the old "VM never subscribes to anything" model. Disabled (nil adapter) until the substrate is fully wired, so inert on every deploy today. (#1152)
...
## v1.10.0 — 2026-07-10
- fix: re-key the bus heartbeat on the processor instance, not the transaction stage, per the org-wide bus-grammar lock-in (#1127). `peregrine_bus` already shipped `Subjects::Penetrator.scanner_heartbeat(tp_id)` (`telemetry.tp.scanner.heartbeat.<tp-id>`), but `ControlPlaneLoop` still published to the flat `telemetry.penetrator.scan.heartbeat` subject scanner#1052 had previously ratified — that shape put the transaction class+stage into a telemetry subject, which the new grammar's processor-vs-transaction rule (the infrastructure repo's `docs/bus-subject-grammar.md`, org-wide bus-grammar lock-in) rules out; #1052's own "do not switch" guidance is superseded by this lock-in. Switched the heartbeat publish to the grammar-correct helper, and added a one-shot `telemetry.tp.scanner.start.<tp-id>` signal at successful `ControlPlaneLoop#start` (built directly off `Subjects.telemetry` — no dedicated gem helper exists yet) so the tp-monitor's fleet-roster KV entry is seeded at boot, not just renewed by the first heartbeat. In-flight transaction id(s) still ride in the payload, not the subject. Both remain no-ops off-bus (identity/tp_id absent) or when the bus publisher is disabled — the GCS `control/` heartbeat stays the durable fallback either way. Env-token + corporate-NATS-org repoint (the rest of #1127's universal deltas) are deferred until infra stands up the corporate substrate as the issue's own precondition. (#1127)
- feat: resolve job identity from the launcher's `BUS_JOB_POINTER` claim-check (#1124). The on-demand landing-zone decision (launcher-consumes, not VM-pulls-one) shipped launcher-side: the launcher hands the VM a clear `ClaimCheck` pointer via instance metadata → tmpfs (`/dev/shm/peregrine/bus-job-pointer.json`, same discipline as the existing `KEYSET` injection) instead of subscribing itself. Added `Bus::JobPointerResolver`, wired into `bin/scan`: opens the sealed blob the pointer references (via the existing `Bus::AdapterEnv` substrate/keyset wiring) and threads `job_id`/`transaction_id`/`environment` into `ENV` ahead of the existing reads. No-op — and today's only live path — when the pointer file is absent; **fails loud** (raises) rather than silently falling back to defaults when the file exists but can't be opened, since a real bus-triggered launch has no other source of truth. `SCAN_PROFILE`/`TARGET_NAME`/`TARGET_URLS` stay on the ENV/CLI path for now — verified (cross-repo, read-only) that the orchestrator's `scan.requested` payload doesn't carry them yet; filed against the orchestrator (not touching that repo) rather than guessing at a shape that doesn't exist. Zero live cost today (`BUS_BUCKET`/`BUS_KEYSET_PATH` still unset in any real deploy). (#1124)

## v1.9.0 — 2026-07-07
- chore: remove the on-VM `Bus::ScanRequestConsumer` — the landing-zone architecture it assumed is not what infra decided. `Bus::ScanRequestConsumer` (merged in #1107, never enabled — no live behavior change) had `bin/scan` itself `Adapter.consume(scan.requested)`. The cross-TP landing-zone decision (covering Scanner/Analyzer/Reporter identically) is the opposite shape: the **launcher** consumes `scan.requested` and hands the job to the VM via instance metadata (the same channel already used for the keyset), and the VM never subscribes to anything — less on-VM bus trust surface, and avoids two competing-consumer reads (launcher + VM) racing on one message. `bin/scan` reverts to its pre-#1106 ENV-first launch path, which is already the final shape once the launcher (out-of-repo, infra-owned) starts injecting the job pointer via env/metadata — no scanner-side consumer code is needed at all. `ScanCompletionPublisher`/`Bus::Publisher` (publish-side) are unaffected. (#1106)
- feat: durable `probe_id` catalog + explicit per-probe run/not-run accounting (#1068). Spec-quality request from the Analyzer seat: `scan-profile/v1` references Scanner probes by a stable id, and needs to distinguish "planned but never attempted" from "not planned" to verify its no-silent-coverage-loss guarantee — neither existed. Added `Probes::Catalog` (`app/services/probes/catalog.rb`): one durable `probe_id` per `ScanOrchestrator::SCANNER_MAP` tool (identity-mapped today), with an append-only `RETIRED` id→id-or-nil map so a retired id stays interpretable in historical profiles/coverage ledgers instead of being deleted. `probe_id` is a third axis, distinct from both the profile YAML `tool` key and the finding-level `probe` open-vocabulary category (docs/probe_contract.md §5) — they happen to be 1:1 today but aren't the same concept. `tool_chain.planned[]`/`executed[]` now carry `probe_id`; a new `tool_chain.probe_accounting[]` gives one explicit `{ probe_id, tool, executed, skip_reason }` entry per planned probe, replacing the implicit "diff planned against executed" the Analyzer would otherwise have had to do (which can't distinguish a probe skipped by control-plane cancellation or an earlier critical-tool abort from one that was simply never planned). `SCHEMA_VERSION` 2.0 → 2.1 (MINOR/additive per the contract's own versioning policy). Not blocking today (gated on the Analyzer's Sight DB per the issue) — this is the Scanner-owned half landing ahead of that dependency. (#1068)
