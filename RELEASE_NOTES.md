...
- fix: rubocop autocorrects and amass Dockerfile.base download URL (#51, #50, #47). CI failure on development from PR #874 merge: 53 autocorrectable rubocop offenses (`Layout/HashAlignment`, `Style/RescueModifier`, `Layout/ExtraSpacing`, `Style/StringLiterals`, `Layout/LineEndStringConcatenationIndentation`) across scan_orchestrator + new scanner/parser files. amass `Dockerfile.base` used `.zip` but v5.1.1 ships as `amass_linux_amd64.tar.gz`; fixed download URL + extraction command. (#51, #50, #47)
