...
- feat: add `reduced` scan profile (baked-tools-only: testssl + ZAP baseline + Nuclei + trufflehog) and register it in the `Scan` model's profile validation (#916). The org-native baked image carries the placed tool set (no nikto/retire/ffuf pending bakery vendoring), so `reduced` is the runnable profile on `txn-scanner-app` until those tools are baked. (#916)
