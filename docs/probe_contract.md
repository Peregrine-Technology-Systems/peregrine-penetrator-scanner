...
- **`probe_id` is a third axis, distinct from `probe` and the tool key (#1068, MINOR, envelope 2.0 → 2.1).** `tool_chain.planned[]`/`executed[]` entries now carry a `probe_id` — the durable identifier `app/services/probes/catalog.rb` (`Probes::Catalog`) assigns per scanner, for the Analyzer's `scan-profile/v1` to select/weight probes against. This is independent of both the profile YAML `tool` key (may vary) and the finding-level `probe` open-vocabulary category from §5 (a classification, not an identity) — today they happen to be in step 1:1, but `probe_id` is the one contractually durable across a tool rename or a probe splitting into several selectable units; retired ids are remapped, never deleted or reused (see the module doc).
- **`tool_chain.probe_accounting[]` makes coverage explicit, not inferred.** One entry per **planned** probe for the scan, `{ probe_id, tool, executed: true|false, skip_reason }` — `executed: false` (e.g. `skip_reason: "not_attempted"`) covers a probe that never got a turn (control-plane cancellation, an earlier critical-tool failure aborting the scan) that `tool_chain.executed` alone cannot distinguish from "not planned." This is the positive signal a "diff planned against executed" set-difference cannot give: consumers get an explicit run/not-run record for every probe instead of inferring absence.
